...
export default function Property({isLoggedIn, Offers}: PropertyProps): JSX.Element {
  ...
  const pageId: string = history.search.replace('?', '');
  let maxID = 100;

  const onReviewSubmit = (rating: number, message: string) => {
    const newRev: Review= {
      'comment': message,
  ...
  return (
    <Route
      render={()=> currentOffer.length > 0 ? currentOffer[0] : <Redirect to='/404'/>}
    />
  );
